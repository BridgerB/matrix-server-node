...
import type { PushRulesContent, PushCondition, PushAction } from "./types/push.ts";
...
const VALID_KINDS = new Set<string>(["override", "content", "room", "sender", "underride"]);
...
export async function getOrInitRules(storage: Storage, userId: UserId): Promise<PushRulesContent> {
  ...
  await storage.setGlobalAccountData(userId, "m.push_rules", defaults as unknown as JsonObject);
  return defaults;
}

export async function saveRules(storage: Storage, userId: UserId, rules: PushRulesContent): Promise<void> {
  await storage.setGlobalAccountData(userId, "m.push_rules", rules as unknown as JsonObject);
}
...
export function evaluatePushRules(rules: PushRulesContent, ctx: EvaluationContext): PushEvalResult {
  ...
  for (const rule of ruleset.override ?? []) {
    ...
    if (checkConditions(rule.conditions ?? [], ctx)) return parseActions(rule.actions);
  }
  ...
  for (const rule of ruleset.content ?? []) {
    ...
    if (rule.conditions && rule.conditions.length > 0) {
      ...
      if (typeof body === "string" && globMatchWordBoundary(rule.pattern, body)) {
...
function checkConditions(conditions: PushCondition[], ctx: EvaluationContext): boolean {
...
function checkCondition(cond: PushCondition, ctx: EvaluationContext): boolean {
  switch (cond.kind) {
    ...
    case "sender_notification_permission": {
      ...
      const threshold = ctx.powerLevels?.notifications?.[cond.key as "room"] ?? 50;
    ...
    case "event_property_contains": {
      ...
      return value.some((item) => jsonValueEquals(item as JsonValue, cond.value));
    }
