...
export function requireFederationAuth(
  ...
  return async (req, next) => {
    ...
    if (params.destination !== serverName) {
      throw forbidden(`Destination mismatch: expected ${serverName}, got ${params.destination}`);
    }
    ...
    if (!pubKey) {
      throw serverNotTrusted(`Could not fetch key ${params.key} from ${params.origin}`);
    }
    ...
    if (req.body !== undefined && req.body !== null && typeof req.body === "object" && Object.keys(req.body as object).length > 0) {
...
function parseXMatrixAuth(header: string): { origin: string; destination: string; key: string; sig: string } | null {
