...
export type Handler = (req: RouterRequest) => RouterResponse | Promise<RouterResponse>;
export type Middleware = (req: RouterRequest, next: Handler) => RouterResponse | Promise<RouterResponse>;
...
export class Router {
  ...
  add(method: string, pattern: string, handler: Handler, ...middleware: Middleware[]): void {
    this.routes.push({
    ...
    if (method === "POST" || method === "PUT" || method === "PATCH" || method === "DELETE") {
      ...
      if (raw.length > 0) {
        ...
        try {
          ...
          respondJson(res, 400, { errcode: "M_BAD_JSON", error: "Could not parse JSON body" });
          return;
        }
      } else {
        body = {};
  ...
  private handleError(res: ServerResponse, err: unknown): void {
    if (err instanceof MatrixError) {
      ...
      respondJson(res, 500, { errcode: "M_UNKNOWN", error: "Internal server error" });
    }
  }
}
