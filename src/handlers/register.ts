...
import type { RegisterRequest, UIAAResponse, LoginResponse, AuthType } from "../types/index.ts";
import { badJson, userInUse, invalidUsername, weakPassword, forbidden, invalidParam } from "../errors.ts";
import { generateToken, generateDeviceId, generateSessionId } from "../crypto.ts";
...
export function postRegister(storage: Storage, serverName: string): Handler {
  return async (req) => {
    ...
    if (!USERNAME_RE.test(localpart)) {
      throw invalidUsername("Username can only contain lowercase letters, digits, and ._=-/");
    }
    ...
    if (body.password.length < MIN_PASSWORD_LENGTH) {
      throw weakPassword(`Password must be at least ${MIN_PASSWORD_LENGTH} characters`);
    }
