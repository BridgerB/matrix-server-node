...
export function getRelations(storage: Storage): Handler {
  return async (req) => {
    ...
    if (!target || target.event.room_id !== roomId) throw notFound("Event not found");
    ...
    const result = await storage.getRelatedEvents(roomId, eventId, relType, eventType, limit, from, dir);
