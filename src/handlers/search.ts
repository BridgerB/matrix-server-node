...
export function postSearch(storage: Storage): Handler {
  return async (req) => {
    ...
    if (!roomEvents?.search_term) throw badJson("Missing search_categories.room_events.search_term");
    ...
    if (roomEvents.filter?.rooms) {
      const filterRooms = new Set(roomEvents.filter.rooms as unknown as RoomId[]);
      roomIds = joinedRooms.filter((r) => filterRooms.has(r));
    }

    const searchResult = await storage.searchRoomEvents(roomIds, searchTerm, keys, limit, from);
    ...
    for (const { event, eventId, streamPos } of searchResult.events) {
      ...
      if (roomEvents.event_context) {
        const beforeResult = await storage.getEventsByRoom(event.room_id, beforeLimit, streamPos, "b");
        const afterResult = await storage.getEventsByRoom(event.room_id, afterLimit, streamPos, "f");

        const eventsBefore = beforeResult.events.map((e) => pduToClientEvent(e.event, e.eventId));
        const eventsAfter = afterResult.events.map((e) => pduToClientEvent(e.event, e.eventId));
        ...
        if (includeProfile) {
          const profileInfo: Record<UserId, { displayname?: string; avatar_url?: string }> = {};
          const allSenders = new Set<UserId>([event.sender, ...eventsBefore.map((e) => e.sender), ...eventsAfter.map((e) => e.sender)]);
          for (const sender of allSenders) {
            const profile = await storage.getProfile(sender);
            if (profile) {
              profileInfo[sender] = { displayname: profile.displayname, avatar_url: profile.avatar_url };
            }
          }
      ...
      if (includeState && !stateMap[event.room_id]) {
        ...
        stateMap[event.room_id] = allState.map((e) => pduToClientEvent(e.event, e.eventId));
      }
    }
    ...
    let groups: Record<string, Record<string, { results: string[]; order: number }>> | undefined;
      ...
      body: {
        search_categories: {
          room_events: {
            ...
            highlights,
            results,
            state: includeState && Object.keys(stateMap).length > 0 ? stateMap : undefined,
            groups,
