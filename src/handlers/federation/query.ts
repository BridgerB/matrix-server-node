    ...
    for (const roomId of publicRoomIds) {
      ...
      for (const [key, event] of room.state_events) {
        if (key.startsWith("m.room.member\0")) {
          if ((event.content as Record<string, unknown>)["membership"] === "join") memberCount++;
        }
      }

      rooms.push({
        room_id: roomId,
        name: nameEvent ? (nameEvent.content as Record<string, unknown>)["name"] : undefined,
        topic: topicEvent ? (topicEvent.content as Record<string, unknown>)["topic"] : undefined,
        canonical_alias: aliasEvent ? (aliasEvent.content as Record<string, unknown>)["alias"] : undefined,
        avatar_url: avatarEvent ? (avatarEvent.content as Record<string, unknown>)["url"] : undefined,
