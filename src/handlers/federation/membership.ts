...
import type { RoomId, UserId, EventId, ServerName, KeyId } from "../../types/index.ts";
import {
  computeEventId, selectAuthEvents, checkEventAuth,
  getMembership,
} from "../../events.ts";
...
export function getMakeJoin(storage: Storage, _serverName: string): Handler {
  return async (req) => {
    ...
    if (createEvent) {
      const federate = (createEvent.content as Record<string, unknown>)["federate"];
  ...
  return async (req) => {
    ...
    if (!room) {
      await storage.importRoomState(
        event.room_id,
        (body.room_version ?? "10") as import("../../types/room-versions.ts").RoomVersion,
        [coSigned],
        [],
      );
    } else {
