...
import type { ServerName, EventId, KeyId, UserId, RoomId } from "../../types/index.ts";
import { computeEventId, computeContentHash, checkEventAuth } from "../../events.ts";
...
export function putFederationSend(
  ...
  return async (req) => {
    ...
    for (const pdu of pdus) {
      ...
      try {
        await processPdu(storage, pdu, eventId, origin, remoteKeyStore, federationClient);
        pduResults[eventId] = {};
      } catch (err) {
        pduResults[eventId] = { error: err instanceof Error ? err.message : "Processing failed" };
      }
    }
...
async function processPdu(
  ...
  for (const keyId of Object.keys(originSigs)) {
    const pubKey = await remoteKeyStore.getServerKey(origin, keyId as KeyId, federationClient);
...
async function processEdu(
  ...
  switch (edu.edu_type) {
    ...
    case "m.receipt": {
      const roomId = content["room_id"] as RoomId;
      const receipts = content["receipts"] as Record<string, Record<string, Record<string, unknown>>> | undefined;
      if (roomId && receipts) {
        for (const [eventId, receiptTypes] of Object.entries(receipts)) {
          for (const [receiptType, users] of Object.entries(receiptTypes)) {
            for (const userId of Object.keys(users)) {
              await storage.setReceipt(roomId, userId as UserId, eventId as EventId, receiptType, Date.now());
            }
          }
        }
      }
