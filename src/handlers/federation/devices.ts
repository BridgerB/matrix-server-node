...
export function postFederationUserDevices(storage: Storage): Handler {
  return async (req) => {
    ...
    const devices: { device_id: string; device_display_name?: string; keys: unknown }[] = [];

    for (const device of allDevices) {
      const keys = await storage.getDeviceKeys(userId, device.device_id as DeviceId);
      if (keys) {
        devices.push({
      ...
      for (const [userId, deviceIds] of Object.entries(body.device_keys)) {
        ...
        if (deviceIds.length === 0) {
          ...
          for (const deviceId of deviceIds) {
            const keys = await storage.getDeviceKeys(userId as UserId, deviceId);
            if (keys) {
...
export function postFederationKeysClaim(storage: Storage): Handler {
  return async (req) => {
    ...
    const oneTimeKeys: Record<UserId, Record<DeviceId, Record<KeyId, unknown>>> = {};
    ...
    if (body.one_time_keys) {
      for (const [userId, devices] of Object.entries(body.one_time_keys)) {
        ...
        for (const [deviceId, algorithm] of Object.entries(devices)) {
          const claimed = await storage.claimOneTimeKey(userId as UserId, deviceId as DeviceId, algorithm);
          if (claimed) {
