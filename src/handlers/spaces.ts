...
export function getSpaceHierarchy(storage: Storage): Handler {
  return async (req) => {
    ...
    const limit = Math.min(Math.max(parseInt(limitStr ?? String(MAX_ROOMS), 10), 1), MAX_ROOMS);
    const maxDepth = Math.max(parseInt(req.query.get("max_depth") ?? "50", 10), 0);
    ...
    const queue: { roomId: RoomId; depth: number }[] = [{ roomId: rootRoomId, depth: 0 }];
    ...
    while (queue.length > 0 && rooms.length < limit) {
      ...
      const canSee = membership === "join" || historyVis === "world_readable" || joinRule === "public";
    ...
    const nextBatch = queue.length > 0 && rooms.length === limit
...
function buildHierarchyRoom(
  room: { state_events: Map<string, import("../types/events.ts").PDU>; room_id: string },
  roomId: RoomId,
  ...
  return {
    room_id: roomId,
    name: nameEvent ? (nameEvent.content as Record<string, unknown>)["name"] as string : undefined,
    topic: topicEvent ? (topicEvent.content as Record<string, unknown>)["topic"] as string : undefined,
    avatar_url: avatarEvent ? (avatarEvent.content as Record<string, unknown>)["url"] as string : undefined,
    canonical_alias: aliasEvent ? (aliasEvent.content as Record<string, unknown>)["alias"] as string : undefined,
    num_joined_members: memberCount,
    world_readable: histVis === "world_readable",
    guest_can_join: guestAccess === "can_join",
    join_rule: joinRulesEvent ? (joinRulesEvent.content as Record<string, unknown>)["join_rule"] as string : undefined,
    room_type: createEvent ? (createEvent.content as Record<string, unknown>)["type"] as string : undefined,
