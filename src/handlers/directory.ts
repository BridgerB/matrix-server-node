...
import type { PublicRoomEntry, PublicRoomsResponse } from "../types/directory.ts";
...
export function getDirectoryRoom(storage: Storage): Handler {
  return async (req) => {
    ...
    return { status: 200, body: { room_id: result.room_id, servers: result.servers } };
  };
}

export function putDirectoryRoom(storage: Storage, serverName: string): Handler {
  return async (req) => {
    const roomAlias = req.params["roomAlias"]! as RoomAlias;
    ...
    if (membership !== "join") throw forbidden("Must be in the room to create an alias");
...
export function deleteDirectoryRoom(storage: Storage, _serverName: string): Handler {
  return async (req) => {
    const roomAlias = req.params["roomAlias"]! as RoomAlias;
    ...
    if (creator !== req.userId) {
      ...
      if (room) {
        ...
        if (userPl < requiredPl) {
          throw forbidden("Must be alias creator or room admin to delete alias");
        }
      } else {
...
async function buildPublicRoomEntry(storage: Storage, roomId: RoomId): Promise<PublicRoomEntry | undefined> {
  const room = await storage.getRoom(roomId);
  ...
  const name = nameEvent ? (nameEvent.content as Record<string, unknown>)["name"] as string | undefined : undefined;

  const topicEvent = room.state_events.get("m.room.topic\0");
  const topic = topicEvent ? (topicEvent.content as Record<string, unknown>)["topic"] as string | undefined : undefined;

  const avatarEvent = room.state_events.get("m.room.avatar\0");
  const avatarUrl = avatarEvent ? (avatarEvent.content as Record<string, unknown>)["url"] as string | undefined : undefined;

  const canonicalEvent = room.state_events.get("m.room.canonical_alias\0");
  const canonicalAlias = canonicalEvent ? (canonicalEvent.content as Record<string, unknown>)["alias"] as string | undefined : undefined;

  const joinRulesEvent = room.state_events.get("m.room.join_rules\0");
  const joinRule = joinRulesEvent ? (joinRulesEvent.content as Record<string, unknown>)["join_rule"] as string | undefined : undefined;

  const historyEvent = room.state_events.get("m.room.history_visibility\0");
  const historyVisibility = historyEvent ? (historyEvent.content as Record<string, unknown>)["history_visibility"] as string | undefined : undefined;

  const guestEvent = room.state_events.get("m.room.guest_access\0");
  const guestAccess = guestEvent ? (guestEvent.content as Record<string, unknown>)["guest_access"] as string | undefined : undefined;

  const createEvent = room.state_events.get("m.room.create\0");
  const roomType = createEvent ? (createEvent.content as Record<string, unknown>)["type"] as string | undefined : undefined;
...
export function getPublicRooms(storage: Storage): Handler {
  return async (req) => {
    const limit = Math.min(parseInt(req.query.get("limit") ?? "20", 10), MAX_PUBLIC_ROOMS);
...
export function postPublicRooms(storage: Storage): Handler {
  return async (req) => {
    ...
    const response = await buildPublicRoomsResponse(storage, limit, since, searchTerm);
    return { status: 200, body: response };
  };
}
...
async function buildPublicRoomsResponse(
  ...
  for (const roomId of publicRoomIds) {
    ...
    if (searchTerm) {
      ...
      const aliasMatch = entry.canonical_alias?.toLowerCase().includes(searchTerm);
  ...
  // Pagination
  const offset = since ? parseInt(Buffer.from(since, "base64url").toString(), 10) : 0;
  const sliced = allEntries.slice(offset, offset + limit);

  const nextOffset = offset + limit;
  const nextBatch = nextOffset < total ? Buffer.from(String(nextOffset)).toString("base64url") : undefined;
  const prevBatch = offset > 0 ? Buffer.from(String(Math.max(0, offset - limit))).toString("base64url") : undefined;
