...
export function getNotifications(storage: Storage): Handler {
  return async (req) => {
    ...
    const allEvents: { event: ReturnType<typeof pduToClientEvent>; roomId: string; streamPos: number; actions: unknown[]; highlight: boolean }[] = [];
    ...
    for (const roomId of joinedRooms) {
      ...
      for (const { event, eventId } of result.events) {
        ...
        if (evalResult.sound) actions.push({ set_tweak: "sound", value: evalResult.sound });
    ...
    const nextToken = sliced.length === limit && sliced.length > 0
