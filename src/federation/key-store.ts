...
export const getServerKey = async (
	...
	const cached = await storage.getServerKeys(serverName, keyId);
	...
	try {
		...
		if (!keys?.server_name || keys.server_name !== serverName) return undefined;
		...
		await storage.storeServerKeys(serverName, keys);
